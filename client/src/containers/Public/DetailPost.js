...
const DetailPost = () => {
    ...
    return (
        <div className="container mx-auto py-4 px-6">
            ...
            {isOpen && (
                <Lightbox
                    mainSrc={data.image[currentImage]}
                    nextSrc={data.image[(currentImage + 1) % data.image.length]}
                    prevSrc={data.image[(currentImage + data.image.length - 1) % data.image.length]}
                    onCloseRequest={closeLightbox}
                    onMovePrevRequest={() =>
                        setCurrentImage((currentImage + data.image.length - 1) % data.image.length)
                    }
                    onMoveNextRequest={() =>
                        setCurrentImage((currentImage + 1) % data.image.length)
                    }
                    imageCaption={`Image ${currentImage + 1} of ${data.image.length}`} // Tùy chỉnh chú thích ảnh nếu cần
                />
            )}
