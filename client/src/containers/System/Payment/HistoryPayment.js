...
const HistoryPayment = () => {
    ...
    useEffect(() => {
        const fetchDepositHistory = async () => {
            try {
                const res = await axios.get('http://localhost:5000/api/v1/user/historyPayment', {
                    headers: {
                        'token': `${token}`,
                    }
                });
                                ...
                                <table className="min-w-full border-collapse border border-gray-200">
                                    <thead>
                                        <tr>
                                            ...
                                            <th className="border border-gray-200 px-4 py-2">Nội dung thanh toán</th>
                                    ...
                                    <tbody>
                                        {currentItems.map((item, index) => (
                                            <tr key={index}>
                                                ...
                                                <td className="border border-gray-200 px-4 py-2 text-center align-middle">{item.content}</td>
                                                <td className="border border-gray-200 px-4 py-2 text-center align-middle">{parseInt(item.balanceAfterTransaction).toLocaleString('vi-VN')}</td>
