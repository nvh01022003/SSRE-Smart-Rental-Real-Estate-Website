...
const getPaymentByTime = async (req, res) => {
    try {
        const { year, month } = req.query;
        console.log("Year:", year, "Month:", month);
        // Kiểm tra input
        ...
        // Gọi service để lấy tổng tiền nạp
        const totalDeposit = await statisticsService.getPaymentByTime(year, month);
        ...
        console.error('Error in getPaymentByTime controller:', error);
        return res.status(500).json({ message: "Internal server error" });
    }
};
